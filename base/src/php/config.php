...
if (env('CFG_SESSION_HANDLER_CLASS') == '\core\session\database') {
  ...
  $CFG->session_database_acquire_lock_timeout = env_as_int('CFG_SESSION_DATABASE_ACQUIRE_LOCK_TIMEOUT', 2);
}
...
if (env('CFG_SESSION_HANDLER_CLASS') == '\core\session\memcached') {
  ...
  $CFG->session_memcached_acquire_lock_timeout = env_as_int('CFG_SESSION_MEMCACHED_ACQUIRE_LOCK_TIMEOUT', 2);
...
// Desabilitado em favor de file-based sessions para melhor reliability
...
// Useful in Docker/NFS environments where file locks may be unreliable.
if (!empty(env('CFG_LOCK_FACTORY', ''))) {
  $CFG->lock_factory = env('CFG_LOCK_FACTORY');
}

// File lock location (used when file lock factory is selected).
$CFG->file_lock_root = env('CFG_FILE_LOCK_ROOT', $CFG->dataroot . '/lock');

// Explicit control over file locking behavior.
$CFG->preventfilelocking = env_as_bool('CFG_PREVENTFILELOCKING', false);
...
if (env('CFG_ENV', 'local') == 'local' && env_as_bool('CFG_DEBUG', false)) {
  ...
  if (!env_as_bool('CFG_DEBUG_WARNINGS', false) && !env_as_bool('CFG_DEBUG_DEPRECATED', false)) {
...
// Increase memory limit to reduce risk of segfault
